...
func TestArchiveTestcaseHandler(t *testing.T) {
	...
	response, err := NewProblemService(biz.NewProblemUsecaseWithStore(repo, repo, &serviceObjectStore{}, repo)).ArchiveTestcase(context.Background(), &problemv1.ArchiveTestcaseRequest{Context: adminContextProto(), ProblemId: 2, TestcaseId: 7})
