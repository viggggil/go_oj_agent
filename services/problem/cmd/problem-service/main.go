...
func main() {
	flagconf := flag.String("conf", "services/problem/configs/config.yaml", "config path")
	flag.Parse()
	...
	if err != nil {
		errorLogger.Error("failed to initialize problem-service", "error", err)
		os.Exit(1)
	}
	defer cleanup()
	...
	if err := app.Run(); err != nil {
		errorLogger.Error("problem-service exited with error", "error", err)
		os.Exit(1)
	}
}
